...
var tokenGenerator = new FirebaseTokenGenerator('sDo0vPzwyQIlDp1U4voPOLH3OIs2ly5zLGLdXQuo');
...
var Pass = function(passport) {
	...
	// -every time- from the DB. Figure out how to store this elsewhere nicely.
	passport.deserializeUser(function(email, done) {
		email = connection.escape(email);
		connection.query("SELECT * FROM users WHERE user_email = " + email, function(err, rows) {
			rows[0].fb_token = tokenGenerator.createToken({username: rows[0].user_nickname, admin: rows[0].user_is_admin});
			done(err, rows[0]);
		});
	});
