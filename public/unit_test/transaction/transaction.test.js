describe('transaction.js', function(){
	...
	before(function(done){
			...
			localTransTest();
			$('#transaction').html('<iframe src="transaction.test.remote.html"></iframe>');
			modelLoaded( 0 );
	...
	describe('createTransaction()', function(){
	   ...
	   it('Can create two transactions', function(){
			...
			trans = createTransaction( "VisualModel", visualActions, trans );
	    ...
	    it('insert update and delete all work locally and remotely', function(){
			...
			expect(JSONEqual( model, testModel )).to.be.true;
	    })
  })
})
...
function modelLoaded( _i, _callback ){
	if( typeof model.loaded == 'boolean' && model.loaded ){
